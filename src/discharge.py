...
def obs_data(station,syear=2000,smon=1,sday=1,eyear=2001,emon=12,eday=31,obs_dir="./obs"):
    ...
    fname =obs_dir+"/"+station+"_Q_Day.Cmd.txt"
    print ( "-- reading observation file: ", fname)
    head = 37 #header lines
    if not os.path.exists(fname):
        ...
        for line in lines[head::]:
            line1    = list(filter(None, re.split(";",line)))      ########  this will also works
            yyyymmdd = list(filter(None, re.split("-",line1[0])))
            # print yyyymmdd
            # line1 = line.split(" ")
            # line2 = line1[0].split(";")
...
#            yyyymmdd = filter(None, re.split("-",line[0]))
            ...
            if start_dt <= datetime.date(int(yyyy),int(mm),int(dd)) and \
                ...
                try:
                    dis[yyyy+mm+dd]=float(line1[2])
                except:
                    dis[yyyy+mm+dd]=-9999.0
            elif last_dt  < datetime.date(int(yyyy),int(mm),int(dd)):
                break
        #---
        start=0
        last=(last_dt-start_dt).days + 1
...
for line in lines[1:100]:
    line2 = line.split(";")
    grdcid.append(line2[0].strip())
    rivers.append(line2[1].strip())
    pnames.append(remove_string(line2[2].strip()))
    x1list.append(int(line2[3]))
    y1list.append(int(line2[4]))
    x2list.append(int(line2[5]))
    y2list.append(int(line2[6]))
...
def make_fig(point):
    ...
    org=obs_data(grdcid[point],syear=syear,eyear=eyear)
    org=np.array(org)
    ...
    # draw simulations
    lines.append(ax1.plot(np.arange(start,last),sim[:,point],label=labels[1],color="#ff8021",linewidth=1.0,alpha=1,zorder=106)[0])
