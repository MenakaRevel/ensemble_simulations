...
def prepare_input():
    ...
    return 0
######################################
def prep_runoff_ensemble(ne,runname,rundir,outdir,
                        syear=2001,eyear=2010,method="simple",
                        beta=0.0,E=0.3,alpha=0.993,tau_s=50.0,
                        distopen=0.0,diststd=0.25):
    """
...
if __name__ == "__main__":
    syear=int(sys.argv[1]) # start year
    eyear=int(sys.argv[2]) # end year
    ne=int(sys.argv[3]) # number of ensemble members
    runname=sys.argv[4] # name of the runoff dataset
    runoff_dir=sys.argv[5] # directory of the runoff dataset
    out_dir=sys.argv[6] # output directory
    method=sys.argv[7] # method of generating the ensemble (e.g., lognormal, normal, simple)
    beta=float(sys.argv[8]) # bias for lognormal distribution
    E=float(sys.argv[9]) # error for lognormal distribution
    alpha=float(sys.argv[10]) # alpha parameter for lognormal distribution
    distopen=float(sys.argv[11]) # mean of the distribution
